...
public class Gui {
   ...
   public void render(GuiGraphics gfx, float p_282611_) {
      ...
      if (Minecraft.useFancyGraphics()) {
         this.renderVignette(gfx, this.minecraft.getCameraEntity());
      } else {
      ...
      if (this.minecraft.options.getCameraType().isFirstPerson()) {
         if (this.minecraft.player.isScoping()) {
            this.renderSpyglassOverlay(gfx, this.scopeScale);
         } else {
            this.scopeScale = 0.5F;
            ItemStack itemstack = this.minecraft.player.getInventory().getArmor(3);
            if (itemstack.is(Blocks.CARVED_PUMPKIN.asItem())) {
               this.renderTextureOverlay(gfx, PUMPKIN_BLUR_LOCATION, 1.0F);
            }
         }
      }

      if (this.minecraft.player.getTicksFrozen() > 0) {
         this.renderTextureOverlay(gfx, POWDER_SNOW_OUTLINE_LOCATION, this.minecraft.player.getPercentFrozen());
      }
      ...
      if (f1 > 0.0F && !this.minecraft.player.hasEffect(MobEffects.CONFUSION)) {
         this.renderPortalOverlay(gfx, f1);
      }

      if (this.minecraft.gameMode.getPlayerMode() == GameType.SPECTATOR) {
         this.spectatorGui.renderHotbar(gfx);
      } else if (!this.minecraft.options.hideGui) {
         this.renderHotbar(p_282611_, gfx, inventoryYpos );
      }

      if (!this.minecraft.options.hideGui) {
         RenderSystem.enableBlend();
         this.renderCrosshair(gfx);
         this.minecraft.getProfiler().push("bossHealth");
         this.bossOverlay.render(gfx);
         this.minecraft.getProfiler().pop();
         
         renderPlayerDummy(gfx,28,120,20);
         
         if (this.minecraft.gameMode.canHurtPlayer()) {
            this.renderPlayerHealth(gfx, inventoryYpos);
            
         }

         this.renderVehicleHealth(gfx);
         ...
         if (playerrideablejumping != null) {
            this.renderJumpMeter(playerrideablejumping, gfx, i);
            
         } else if (this.minecraft.gameMode.hasExperience()) {
            this.renderExperienceBar(gfx, i, inventoryYpos);
            
         }

         if (this.minecraft.gameMode.getPlayerMode() != GameType.SPECTATOR) {
            this.renderSelectedItemName(gfx, inventoryYpos);
            
         } else if (this.minecraft.player.isSpectator()) {
            this.spectatorGui.renderTooltip(gfx);
         }
      }
      ...
      if (this.minecraft.player.getSleepTimer() > 0) {
         ...
         gfx.fill(RenderType.guiOverlay(), 0, 0, this.screenWidth, this.screenHeight, j);
         this.minecraft.getProfiler().pop();
      }

      if (this.minecraft.isDemo()) {
         this.renderDemoOverlay(gfx);
      }
      ...
      if (!this.minecraft.options.hideGui) {
         if (this.overlayMessageString != null && this.overlayMessageTime > 0) {
            ...
            if (j1 > 8) {
               gfx.pose().pushPose();
               gfx.pose().translate((float)(this.screenWidth / 2), (float)(this.screenHeight - 68), 0.0F);
               int l1 = 16777215;
               ...
               this.drawBackdrop(gfx, font, -4, l, 16777215 | k);
               gfx.drawString(font, this.overlayMessageString, -l / 2, -4, l1 | k);
               gfx.pose().popPose();
            }
         ...
         if (this.title != null && this.titleTime > 0) {
            ...
            if (k1 > 8) {
               gfx.pose().pushPose();
               gfx.pose().translate((float)(this.screenWidth / 2), (float)(this.screenHeight / 2), 0.0F);
               RenderSystem.enableBlend();
               gfx.pose().pushPose();
               gfx.pose().scale(4.0F, 4.0F, 4.0F);
               int i2 = k1 << 24 & -16777216;
               int j2 = font.width(this.title);
               this.drawBackdrop(gfx, font, -10, j2, 16777215 | i2);
               gfx.drawString(font, this.title, -j2 / 2, -10, 16777215 | i2);
               gfx.pose().popPose();
               if (this.subtitle != null) {
                  gfx.pose().pushPose();
                  gfx.pose().scale(2.0F, 2.0F, 2.0F);
                  int l2 = font.width(this.subtitle);
                  this.drawBackdrop(gfx, font, 5, l2, 16777215 | i2);
                  gfx.drawString(font, this.subtitle, -l2 / 2, 5, 16777215 | i2);
                  gfx.pose().popPose();
               }
         ...
         this.subtitleOverlay.render(gfx);
         ...
         if (objective1 != null) {
            this.displayScoreboardSidebar(gfx, objective1);
         }
         ...
         this.chat.render(gfx, this.tickCount, i3, i1);
         this.minecraft.getProfiler().pop();
         ...
         if (!this.minecraft.options.keyPlayerList.isDown() || this.minecraft.isLocalServer() && this.minecraft.player.connection.getListedOnlinePlayers().size() <= 1 && objective1 == null) {
            ...
            this.tabList.render(gfx, this.screenWidth, scoreboard, objective1);
         }

         this.renderSavingIndicator(gfx);
   ...
   public void renderExperienceBar(GuiGraphics p_281906_, int p_282731_, int ypos) {
      ...
      if (i > 0) {
         ...
         //y-positioning
         int y = this.screenHeight - ypos - 8;
         
         p_281906_.blit(GUI_ICONS_LOCATION, p_282731_, y, 0, 64, 182, 5);
         if (k > 0) {
            p_281906_.blit(GUI_ICONS_LOCATION, p_282731_, y, 0, 69, k, 5);
         }
      }
      ...
      if (this.minecraft.player.experienceLevel > 0) {
         ...
         //y-positioning
         int y = this.screenHeight - 18 - ypos;
         
         p_281906_.drawString(this.getFont(), s, i1 + 1, y, 0, false);
         p_281906_.drawString(this.getFont(), s, i1 - 1, y, 0, false);
         p_281906_.drawString(this.getFont(), s, i1, y + 1, 0, false);
         p_281906_.drawString(this.getFont(), s, i1, y - 1, 0, false);
         p_281906_.drawString(this.getFont(), s, i1, y, 8453920, false);
         this.minecraft.getProfiler().pop();
      }

   }

   public void renderSelectedItemName(GuiGraphics p_283501_, int ypos) {
      ...
      if (this.toolHighlightTimer > 0 && !this.lastToolHighlight.isEmpty()) {
         ...
         int k = this.screenHeight - 40 - ypos;
         if (!this.minecraft.gameMode.canHurtPlayer()) {
            k += 14;
         }
   ...
   private void renderPlayerDummy(GuiGraphics gfx, int tlMargin, int lookAtX, int lookAtY) {
	   InventoryScreen.renderEntityInInventoryFollowsMouse(gfx, this.leftPos+25+tlMargin, this.topPos + 45+tlMargin, 20, (float)(this.leftPos + 88 - lookAtX), (float)(this.topPos + 45 - 30 - lookAtY), this.minecraft.player);
   } 
   
   private void renderPlayerHealth(GuiGraphics p_283143_, int ypos) {
      Player player = this.getCameraPlayer();
      if (player != null) {
         ...
         //y-positioning
         int k1 = this.screenHeight - 19 - ypos;
   ...
   private void renderSlot(GuiGraphics gfx, int p_283213_, int p_281301_, float p_281885_, Player player, ItemStack stack, int p_283261_) {
      if (!stack.isEmpty()) {
         float f = (float)stack.getPopTime() - p_281885_;
         if (f > 0.0F) {
            float f1 = 1.0F + f / 5.0F;
            gfx.pose().pushPose();
            gfx.pose().translate((float)(p_283213_ + 8), (float)(p_281301_ + 12), 10.0F);
            gfx.pose().scale(1.0F / f1, (f1 + 1.0F) / 2.0F, 1.0F);
            gfx.pose().translate((float)(-(p_283213_ + 8)), (float)(-(p_281301_ + 12)), 10.0F);
         }

         gfx.renderItem(player, stack, p_283213_, p_281301_, p_283261_);
         if (f > 0.0F) {
            gfx.pose().popPose();
         }

         gfx.renderItemDecorations(this.minecraft.font, stack, p_283213_, p_281301_);
      }
   }
